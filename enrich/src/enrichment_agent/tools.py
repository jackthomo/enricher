...
async def search(
    ...
    logger.info(
        ...
        extra={"query": query[:200], "max_results": configuration.max_search_results},
    )
    result = await wrapped.ainvoke({"query": query})
    logger.info(
        "tool.search: completed",
        extra={"query": query[:200], "result_count": len(result or [])},
    )
...
async def scrape_website(
    ...
    *,
    ...
    """
    logger.info("tool.scrape_website: fetching", extra={"url": url})
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as response:
            content = await response.text()
            logger.info(
                "tool.scrape_website: fetched",
                extra={"url": url, "status": response.status, "length": len(content)},
            )
    ...
    logger.info("tool.scrape_website: summarizing", extra={"url": url})
    result = await raw_model.ainvoke(p)
    summary = str(result.content)
    logger.info(
        "tool.scrape_website: summarized",
        extra={"url": url, "summary_chars": len(summary)},
    )
    return summary
